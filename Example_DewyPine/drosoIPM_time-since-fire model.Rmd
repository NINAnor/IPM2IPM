...
```{r data handling}
# the data are from a species adapted to fire regimes. The variables on TSF (=time since fire) express that. In this analysis we account for the fire-ecology by including TSF as a covariate in the vital rate models
# Order the levels of TSF categories
dlData$TSF=factor(dlData$TSF,levels = c("one", "two", "three",">three"))
```
...
# plot survival
dlData %>%
  ggplot(aes(x=size, y=surv, color=TSF)) + 
  geom_jitter(height=0.1)

# doing some pre model selection
surv0=glmer(surv~1+(1|year),family=binomial,data=dlData)
surv1=glmer(surv~size+(1|year),family=binomial,data=dlData)
surv2=glmer(surv~size+TSF+(1|year),family=binomial,data=dlData)
surv3=glmer(surv~size+TSF+size*TSF+(1|year),family=binomial,data=dlData)

anova(surv0,surv1,surv2,surv3, test="Chisq")
# we proceed with a an additive model
rm(surv0,surv1,surv2,surv3)

# Define the model and the error family
surv.modelFormula <- cbind(surv, 1 - surv) ~ size + TSF
...
# plot growth
dlData %>%
  ggplot(aes(x=size, y=sizeNext, color=TSF)) +
  geom_point()

# doing some pre model selection
gr0=lmer(sizeNext~1+(1|year),data=dlData[!is.na(dlData$size)&!is.na(dlData$sizeNext),])
gr1=lmer(sizeNext~size+(1|year),data=dlData[!is.na(dlData$size)&!is.na(dlData$sizeNext),])
gr2=lmer(sizeNext~size+TSF+(1|year),data=dlData[!is.na(dlData$size)&!is.na(dlData$sizeNext),])
gr3=lmer(sizeNext~size+TSF+size*TSF+(1|year),data=dlData[!is.na(dlData$size)&!is.na(dlData$sizeNext),])

anova(gr0,gr1,gr2,gr3,test="Chisq")
# we proceed with a an interactive model
rm(gr0,gr1,gr2,gr3)

# Define the model (error family not needed because it is assumed to be Gaussian if not specified)
growth.modelFormula <- sizeNext ~ size*TSF
# Sub-set the data frame so only the columns with relevant data are included
...
# plot flowering
dlData %>%
  ggplot(aes(x=size, y=fl, color=TSF)) + 
    geom_jitter(height=0.1)

# pre model selection
fl0=glmer(fl~1+(1|year),family=binomial,data=dlData)
fl1=glmer(fl~size+(1|year),family=binomial,data=dlData)
fl2=glmer(fl~size+TSF+(1|year),family=binomial,data=dlData)
fl3=glmer(fl~size+TSF+size*TSF+(1|year),family=binomial,data=dlData)
...
# Define the model and the error family
fec1.modelFormula <- cbind(fl, 1 - fl) ~ size*TSF
...
# plot flower stalks
dlData %>%
  ggplot(aes(x=size, y=fs, color=TSF)) + 
    geom_point()
# pre model selection
fs0=glmer(fs~1+(1|year),family=poisson,data=dlData)
fs1=glmer(fs~size+(1|year),family=poisson,data=dlData)
fs2=glmer(fs~size+TSF+(1|year),family=poisson,data=dlData)
fs3=glmer(fs~size+TSF+size*TSF+(1|year),family=poisson,data=dlData)
...
dlData %>%
  ggplot(aes(x=size, y=fps, color=TSF)) + 
    geom_point()
# pre model selection
fps0=glmer(fps~1+(1|year),family=poisson,data=dlData)
fps1=glmer(fps~size+(1|year),family=poisson,data=dlData)
fps2=glmer(fps~size+TSF+(1|year),family=poisson,data=dlData)
fps3=glmer(fps~size+TSF+size*TSF+(1|year),family=poisson,data=dlData)

anova(fps0,fps1,fps2,fps3,test="Chisq")
# we proceed with a an additive model
rm(fps0,fps1,fps2,fps3)

# Define the model and the error family
fec3.modelFormula <- fps ~ size + TSF
fec3.errorFamily <- poisson(link = "log")
# Sub-set the data frame so only the columns with relevant data are included
fec3.inputData <- dlData[!is.na(dlData$fps) & !is.na(dlData$size), ]

# does seedling size vary with TSF?
dlData %>%
  ggplot(aes(x=TSFcont, y=sds)) + 
    geom_point()

dlData %>%
  ggplot(aes(x=TSF, y=sds)) + 
    geom_violin()
# pre model selection
sds0=lmer(sds~1+(1|year),data=dlData)
sds1=lmer(sds~TSF+(1|year),data=dlData)

anova(sds0,sds1,test="Chisq")
# we proceed with a the TSF model
rm(sds0,sds1)

# Define the model and the error family
fec4.modelFormula <- sds ~ TSF
# Sub-set the data frame so only the columns with relevant data are included
fec4.inputData <- dlData[!is.na(dlData$sds) & !is.na(dlData$TSF), ]
```
...
ipmOut <- ipmNIMBLE(mcmcParams = mcmcParams,
                    ...
                    fec4.modelFormula = fec4.modelFormula, fec4.inputData = fec4.inputData
)
```
...
# Plot the marginal posterior distributions of the regression coefficients associated with the sub-model
...
plot(ipmOut$growth$parameterSamples)
...
testData <- data.frame(size = rep(seq(min(c(dlData$size,dlData$sizeNext),na.rm=T)*0.9,
                                  max(c(dlData$size,dlData$sizeNext),na.rm=T)*1.1, 
                                  length.out = 100),
                                  4),
                       TSF = rep(c("one", "two", "three", ">three"),each=100))
