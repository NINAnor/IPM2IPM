...
```{r dataset}
# Download the data
dlData <- read.csv("../example data/dataDroso.csv", header=T)
...
# Sub-set the data frame so only the columns with relevant data are included
fec1.inputData <- dlData[!is.na(dlData$flo.if) & !is.na(dlData$size), ]

# Define the model and the error family
fec2.modelFormula <- flo.no ~ size
fec2.errorFamily <- poisson(link = "log")
# Sub-set the data frame so only the columns with relevant data are included
fec2.inputData <- dlData[!is.na(dlData$flo.no) & !is.na(dlData$size), ]
```
...
# Sub-set the data frame so only the columns with relevant data are included
clo1.inputData <- dlData[!is.na(dlData$cloning) & !is.na(dlData$size), ]

# Define the model and the error family
clo2.modelFormula <- clonesNext ~ size
clo2.errorFamily <- poisson(link = "log")
# Sub-set the data frame so only the columns with relevant data are included
clo2.inputData <- dlData[!is.na(dlData$clonesNext) & !is.na(dlData$size), ]

# Define the model and the error family
clo3.modelFormula <- sizeNext ~ size
# Sub-set the data frame so only the columns with relevant data are included
clo3.inputData <- dlData[dlData$offspringNext=='clonal', ]
...
fecsizeMean <- mean(dlData$sizeNext[dlData$offspringNext=='sexual'], na.rm=T)
fecsizeSD <- sd(dlData$sizeNext[dlData$offspringNext=='sexual'], na.rm=T)
