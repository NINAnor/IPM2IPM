...
source("C:/Users/kate.matthews/OneDrive - NINA/NINA Projects/IPM2IPM/Scripts/megaMatrixAssembly.R")
```

## Example Dataset

This example uses individual-based data on barnacle geese (branta leucopsis) collected at their breeding grounds at Ny-Ålesund, Svalbard between 1991 and 2017. See Layton-Matthews et al. (2019, Journal Animal Ecology) for details on the data collection methods: https://doi.org/10.1111/1365-2656.13001. Demographic data (exData.csv file) includes individual (N=1669) observational data, with columns describing age class (a, fledging = 0 & adult = 1), whether an individual survived from year t to t+1 (surv), body mass in grams for year t (mass) and t+1 (mass_next). Fecundity is recorded by two parameters: whether females had any offspring or not (repF, 0 or 1) and the number of fledgings per female for those that bred successfully (Nfledgling).

```{r loadData}
# Load data and show a snippet of the dataframe
exData <- read.csv("exData.csv",sep=";")
...
surv_f.inputData <- exData[!is.na(exData$surv) & !is.na(exData$mass) & exData$a==0, ]
...
# Sub-set the data frame so only the columns with relevant data are included
surv_a.inputData <- exData[!is.na(exData$surv) & !is.na(exData$mass) & exData$a==1, ]
...
# Sub-set the data frame so only the columns with relevant data are included
growth_f.inputData <- exData[!is.na(exData$mass_next) & !is.na(exData$mass) & exData$a==0, ]

# Define the model (error family not needed because it is assumed to be Gaussian if not specified)
growth_a.modelFormula <- mass_next ~ mass
# Sub-set the data frame so only the columns with relevant data are included
growth_a.inputData <- exData[!is.na(exData$mass_next) & !is.na(exData$mass) & exData$a==1, ]
...
# Sub-set the data frame so only the columns with relevant data are included
fec.inputData <- exData[!is.na(exData$Nfledglings) & !is.na(exData$mass_next) & exData$a==1, ]
```
...
ipmOut <- ipmNIMBLE(mcmcParams = mcmcParams,
  ...
  surv_f.modelFormula = surv_f.modelFormula, surv_f.errorFamily = surv_f.errorFamily, surv_f.inputData = surv_f.inputData,
  surv_a.modelFormula = surv_a.modelFormula, surv_a.errorFamily = surv_a.errorFamily, surv_a.inputData = surv_a.inputData,

  # Define the growth sub-models
  growth_f.modelFormula = growth_f.modelFormula, growth_f.inputData = growth_f.inputData,
  growth_a.modelFormula = growth_a.modelFormula, growth_a.inputData = growth_a.inputData,
...
setNames(
  c(ipmOut$surv_a$rSquared$mean, ipmOut$surv_a$rSquared$quant0.025, ipmOut$surv_a$rSquared$quant0.975),
...
growthMat_f <- ipmKernelEvaluation(ipmOut, testData, testData$mass, growthKern_f, cellWidth = cellWidth)
growthMat_a <- ipmKernelEvaluation(ipmOut, testData, testData$mass, growthKern_a, cellWidth = cellWidth)

# Plot the expected values of the growth kernel matrix - for fledglings
image(testData$mass, testData$mass, t(growthMat_f$expected), xlab = "mass(t)", ylab = "mass(t + 1)")

# Plot the expected values of the growth kernel matrix - for adults
image(testData$mass, testData$mass, t(growthMat_a$expected), xlab = "mass(t)", ylab = "mass(t + 1)")

# Pull out a specific sample from the MCMC samples e.g., 10th (for adults)
image(testData$mass, testData$mass, t(growthMat_a$samples[, , 10]), xlab = "mass(t)", ylab = "mass(t + 1)")
...
growthSurvKern_f <- function(x, surv_f.response, growth_f.response, growth_f.sd, cellWidth) {
  surv_f.response * growthKern_f(x, growth_f.response, growth_f.sd, cellWidth)
}
growthSurvMat_f <- ipmKernelEvaluation(ipmOut, testData, testData$mass, growthSurvKern_f, cellWidth = cellWidth)

# Plot the expected values of the growth survival kernel matrix
image(testData$mass, testData$mass, t(growthSurvMat_f$expected), xlab = "size(t)", ylab = "size(t + 1)", main="fledgling survival and growth")

# Evaluate the growth/survival kernel - adults
growthSurvKern_a <- function(x, surv_a.response, growth_a.response, growth_a.sd, cellWidth) {
  surv_a.response * growthKern_a(x, growth_a.response, growth_a.sd, cellWidth)
}
growthSurvMat_a <- ipmKernelEvaluation(ipmOut, testData, testData$mass, growthSurvKern_a, cellWidth = cellWidth)

# Plot the expected values of the growth survival kernel matrix
image(testData$mass, testData$mass, t(growthSurvMat_a$expected), xlab = "size(t)", ylab = "size(t + 1)", main="adult survival and growth")
...
recKern <- function(x, surv_a.response, rep.response, fec.response, recsizeMean, recsizeSD, cellWidth) {

  surv_a.response * rep.response * fec.response * sexratio * dnorm(x, recsizeMean, recsizeSD) * cellWidth
...
image(testData$mass, testData$mass, t(recMat$expected), xlab = "mass(t)", ylab = "mass(t + 1)")
...
FullOut <- megaMatrixAssembly(stgMatrix, ipmKernels, states, kernelFrom, kernelTo)
...
ggplot(data = data.frame(growthRate = fullMatAnalysis$assymPopGrowth)) + 
  ...
  geom_vline(xintercept = fullMatAnalysis$assymPopGrowthSummary[4], linetype = "dashed") + 
  geom_vline(xintercept = fullMatAnalysis$assymPopGrowthSummary[5], linetype = "dashed") +
  geom_vline(xintercept = fullMatAnalysis$assymPopGrowthSummary[1]) +
  theme_classic() + ylab("Count") + xlab("Asymptotic Growth Rate")

ggplot(data = cbind(data.frame(size = testData$mass), fullMatAnalysis$stableDistSummary[1:100,])) +
...
ggplot(data = cbind(data.frame(size = testData$mass), fullMatAnalysis$stableDistSummary[101:200,])) +
  geom_ribbon(aes(x = size, ymin = lower95cred, ymax = upper95cred), fill = "#69b3a2", color = "#e9ecef") +
  geom_line(aes(x = size, y = mean)) +
...
image(testData$mass, testData$mass, t(growthSurvMat$expected), xlab = "size(t)", ylab = "size(t + 1)","")
image(testData$mass, testData$mass, t(fecMat$expected), xlab = "size(t)", ylab = "size(t + 1)")
image(testData$mass, testData$mass, t(cloMat$expected), xlab = "size(t)", ylab = "size(t + 1)")
image(testData$mass, testData$mass, t(fullMat$expected), xlab = "size(t)", ylab = "size(t + 1)")

# Plot the expected values of the full kernel matrix
image.plot(t(FullOut$samples[,,1]))
image.plot(t(FullOut$expected))
