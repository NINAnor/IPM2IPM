...
We next define a data set for which we want to make predictions from.  In this example, we want to make predictions of the probabilities of arriving at a size in 2002 given a size in 2001 so we can use the 'seq' function in R to make a sequence of sizes that wish to use as 2001 values.  This will be used as input data for the vital-rate model predictions ('response' and 'linpred') described earlier.  Our models here all only use 'size' as a predictor variable so we only need to have size in the prediction data set.  However, if our vital rate models had other covariates (such as temperature or precipitation) then we would need to provide those in our prediction data set too.
...
We define the growth kernel for this example would by the following kernel function:
```{r growthKern}
...
The goal of the IPM2IPM project was to integrate the estimation of trait-dependent vital rates with the analysis of population-level data on trait distribution and numbers. We have not yet developed code for full, simulataneous integration, but below illustrate one ad-hoc approach on integration using "empirical priors". 
In practice, this approach uses the joint posterior distribution of all vital rate parameters as prior distributions in a second modelling step aimed at estimating population size and trait structure over time. This second model also extracts additional information on population-level processes from observational data on population size and trait distributions and is based on the code in "Scripts/EmpiricalPrior.R". 

### Population-level data

First, we need to prepare our population-level data.In lack of additional data, we here treat out input data, ``vpData``, as our population-level observations. 
The population-level data needs to be reformatted into a matrix of observed trait densities (``obsTraitDensity``), where each row corresponds to a trait bin ``i`` and each column corresponds to a time step ``t``. The entries of the matrix are the numbers of individuals observed at time step ``t`` and whose measured trait value falls into trait class ``i``. 

``` {r pop level data prep}
## Define trait bins
numTraitBins <- 10
traitThresholds <- seq(min(c(vpData$size, vpData$sizeNext), na.rm = T)*0.9,
    max(c(vpData$size, vpData$sizeNext), na.rm = T)*1.1, 
    length.out = numTraitBins + 1)
traitBins <- data.frame(Index = 1:numTraitBins, 
                        lowerT = traitThresholds[1:numTraitBins],
                        upperT = traitThresholds[2:(numTraitBins+1)])

## Add trait bins to data
vpData$traitBin <- NA
for(i in 1:nrow(vpData)){
  
  if(!is.na(vpData$size[i])){
      vpData$traitBin[i] <- which(vpData$size[i] > traitBins$lowerT & vpData$size[i] < traitBins$upperT)
  }else{
    vpData$traitBin[i] <- NA
  }
}

## Convert data to trait density matrix
obsTraitDensity <- vpData %>%
  dplyr::mutate(timeStep = dplyr::case_when(trans == "09_10" ~ 1,
                                            trans == "10_11" ~ 2,
                                            trans == "11_12" ~ 3)) %>%
  group_by(traitBin, timeStep) %>% 
  summarise(count = n(), .groups = "keep") %>%
  reshape2::acast(traitBin ~ timeStep)

obsTraitDensity[which(is.na(obsTraitDensity))] <- 0
obsTraitDensity

```
At the moment, the trait density matrix still includes individuals without trait measurements (``NA`` category). Before removing those, we make sure to count them and calculate what proportion of observed individuals did have trait values recorded: 

```{r}
## Number with NA trait values
numTraitNA <- sum(obsTraitDensity["NA",])

## Number and proportion with trait values
numObsIndividuals <- sum(obsTraitDensity) - numTraitNA
propObsTraits <- numObsIndividuals / (numObsIndividuals + numTraitNA)
  
## Trait density matrix without missing values
obsTraitDensity <- obsTraitDensity[which(rownames(obsTraitDensity) != "NA"),]
obsTraitDensity

```

### Joint posteriors from vital rate models

Next we need to assemble the joint posterior for all vital rate parameters in a matrix ``vitalRateParams`` whose rows correspond to posterior samples and whose columns correspond to individual parameters. 

```{r}
## Assemble joint posterior
for(i in 1:length(ipmOut)){
  if(i == 1){
    vitalRateParam <- as.matrix(ipmOut[[i]]$parameterSamples)
  }else{
    vitalRateParam <- cbind(vitalRateParam, as.matrix(ipmOut[[i]]$parameterSamples))
  }
}

## List vital rate parameters
vitalRateParam_List <- colnames(vitalRateParam)
numVitalParams <- length(vitalRateParam_List)
```

### NIMBLE function for projection matrix assembly

In the next step, we need to write a nimbleFunction that we can use within out NIMBLE population model to assemble a projection matrix with ``numTraitBins`` trait classes based on our vital rate parameters. 
We do not have a generic implementation of this function yet, so as of now this needs to be written specifically for each study/population. 

```{r}

## Define midpoints of trait binds for kernel evaluation
midPointTraitBins <- (traitBins$lowerT + traitBins$upperT)/2

## 
projCalc_viola <- function(interceptsurvCoeff,
                           sizesurvCoeff,
                           interceptgrowthCoeff,
                           sizeNextgrowthPrec,
                           sizegrowthCoeff,
                           interceptfec1Coeff,
                           sizefec1Coeff,
  ...
  ## Assemble survival-growth transition matrix
  GS_mat <- matrix(0, nrow = numTraitBins, ncol = numTraitBins)
  
  for(i in 1:numTraitBins){
    
    sizeFrom <- midPointTraitBins[i]
    surv_i <- plogis(qlogis(interceptsurvCoeff) + sizesurvCoeff*sizeFrom)
      
    for(j in 1:numTraitBins){

      sizeTo <- midPointTraitBin[j]
      
      growth_ij
      
      GS_mat[i, j] <- surv_i * growth_ij
    }
  }
  ## Assemble reproduction matrix
  
  ## Assemble clonality matrix
